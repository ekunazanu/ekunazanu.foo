...
for (let i = 80; i < 640; i+= 80)
    rowsWaveRatios.push(1 + i);
const waveRatios = [1, 16/15, 9/8, 6/5, 5/4, 4/3, 45/32, 3/2, 8/5, 5/3, 9/5, 15/8, 2/1];
drawTimeGrid(canvasIntervalRatioWaves, rowsWaveRatios, 98.46, "#ccc", "#ccc", 2, canvasWidth, 640, -49);
for (let i = 0; i < 13; i++) {
...
function drawWaveVertical(canvas, frequency, amplitude, xOffset = 101, stroke = "#000", height = 640, strokewidth = 2) {
    canvas.beginPath();
    canvas.moveTo(xOffset, height);
    for (let y = height; y > 0; y--) {
        const x = xOffset + amplitude * 90 * Math.sin(frequency * (height - y) * Math.PI / 32000);
        canvas.lineTo(x, y);
    }
    canvas.strokeStyle = stroke;
    canvas.lineWidth = strokewidth;
