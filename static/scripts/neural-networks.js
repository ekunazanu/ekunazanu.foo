...
class Model {
    ...
    constructor(dataset, lossFunction, activationFunction, dataDimensions, outputDimensions, hiddenLayers = []) {
    ...
    train(learningRate = 0.001, epochs = 1) {
...
class Plot {
    ...
    drawPlotAxes(xLabel, yLabel, color = "#000", lineWidth = 2, textOffset = TEXTOFFSET) {
    ...
    drawPlotPoint(point, color = "#000", radius = 5) {
...
const plotTwoDimensionsLinearStatic = new Plot("canvasTwoDimensionsLinearStatic", datasetTwoDimensionsLinear, 0, 1, "AREA", "PRICE", false);
plotTwoDimensionsLinearStatic.drawPlotPoints();
plotTwoDimensionsLinearStatic.drawPlotAxes();

const plotTwoDimensionsLinearLineStatic = new Plot("canvasTwoDimensionsLinearLineStatic", datasetTwoDimensionsLinear, 0, 1, "AREA", "PRICE", false);
plotTwoDimensionsLinearLineStatic.drawPlotPoints();
plotTwoDimensionsLinearLineStatic.drawPlotAxes();
plotTwoDimensionsLinearLineStatic.drawLinearRegressionLine(SIMPLE_2D_LINEAR_W1, SIMPLE_2D_LINEAR_W2, true, undefined, undefined, "#aaa");

const plotTwoDimensionsLinear = new Plot("canvasTwoDimensionsLinear", datasetTwoDimensionsLinear, 0, 1, "AREA", "PRICE", false);
...
const modelLoss = new Model(datasetTwoDimensionsLinear, errorSquared, identityFunction, [0], [1]);
const plotTwoDimensionsLinearLoss = new Plot("canvasTwoDimensionsLinearLoss", datasetTwoDimensionsLinear, 0, 1, "AREA", "PRICE", false, 400);
...
function updateLossLandscape() {
    ...
    modelLoss.weights[0][0] = modelLoss.denormalizeWeights([[[Number(sliderLossLandscapeW1.value), Number(sliderLossLandscapeW2.value)]]], false, true)[0];
    varLossLandscapeLoss = modelLoss.calculateLoss();
    modelLoss.train(0.005, 1);
    console.log(modelLoss.denormalizeWeights(modelLoss.weights)[0], modelLoss.denormalizeWeights(modelLoss.weights, true)[0]);
    plotLossLandscape.drawPlotPoint([Number(sliderLossLandscapeW1.value), Number(sliderLossLandscapeW2.value)]);
    plotLossLandscape.drawPlotAxes();
    plotLossLandscapePlotW1.drawPlotPoint([Number(sliderLossLandscapeW1.value), varLossLandscapeLoss]);
    plotLossLandscapePlotW2.drawPlotPoint([Number(sliderLossLandscapeW2.value), varLossLandscapeLoss]);
...
function drawLines(canvas, coordinates, color = "#000", lineWidth = 2) {
