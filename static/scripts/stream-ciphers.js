...
const EG_CHACHASTRUCT = createEncryptionStreams(EG_CHACHAINITBLOCK, 4)
const EG_DATA_STREAMS = [HEXCHECKER, HEXVSTRIPE2, HEXHSTRIPE2];
...
function updateCaesar() {
    ...
    varCaesarText = inputCaesar.value;
    let varCaesarCiphertext = shiftLetters(varCaesarText, varCaesarKey);
    let varCaesarPlaintext = shiftLetters(varCaesarCiphertext, varCaesarKey, -1);
    canvasCaesar.clearRect(0, 0, WIDTH, 500);
    canvasCaesarDecrypt.clearRect(0, 0, WIDTH, 500);
    drawLetterChain(canvasCaesar, varCaesarText);
    drawShiftArrows(canvasCaesar, varCaesarKey, Math.min(varCaesarText.length, LETTERBOXLENGTH));
    drawLetterChain(canvasCaesar, varCaesarCiphertext, 275);
    drawLetterChain(canvasCaesarDecrypt, varCaesarCiphertext);
    drawShiftArrows(canvasCaesarDecrypt, varCaesarKey, Math.min(varCaesarText.length, LETTERBOXLENGTH), "-");
    drawLetterChain(canvasCaesarDecrypt, varCaesarPlaintext, 275);
    spanCaesar.innerHTML = varCaesarKey;
}
updateCaesar();


let varRot13Text = "Encrypting stuff";
const varRot13Key = [13];
...
function updateRot13() {
    varRot13Text = inputRot13.value;
    let varRot13Ciphertext = shiftLetters(varRot13Text, varRot13Key);
    let varRot13Plaintext = shiftLetters(varRot13Ciphertext, varRot13Key);
    canvasRot13.clearRect(0, 0, WIDTH, 650);
    drawLetterChain(canvasRot13, varRot13Text);
    drawShiftArrows(canvasRot13, varRot13Key, Math.min(varRot13Text.length, LETTERBOXLENGTH));
    drawLetterChain(canvasRot13, varRot13Ciphertext, 275);
    drawShiftArrows(canvasRot13, varRot13Key, Math.min(varRot13Text.length, LETTERBOXLENGTH), "+", 395);
    drawLetterChain(canvasRot13, varRot13Plaintext, 535);
    spanRot13.innerHTML = varRot13Key;
}
updateRot13();
...
function updateVignere() {
    varVignereText = inputVignere.value;
    varVignereKey = getArrayFromString(inputVignereKey.value);
    let varVignereCiphertext = shiftLetters(varVignereText, varVignereKey);
    let varVignerePlaintext = shiftLetters(varVignereCiphertext, varVignereKey, -1);
    canvasVignere.clearRect(0, 0, WIDTH, 500);
    canvasVignereDecrypt.clearRect(0, 0, WIDTH, 500);
    drawLetterChain(canvasVignere, varVignereText);
    drawShiftArrows(canvasVignere, varVignereKey, Math.min(varVignereText.length, LETTERBOXLENGTH));
    drawLetterChain(canvasVignere, varVignereCiphertext, 275);
    drawLetterChain(canvasVignereDecrypt, varVignereCiphertext);
    drawShiftArrows(canvasVignereDecrypt, varVignereKey, Math.min(varVignereText.length, LETTERBOXLENGTH), "-");
    drawLetterChain(canvasVignereDecrypt, varVignerePlaintext, 275);
    spanVignere.innerHTML = varVignereKey;
}
updateVignere();
...
function updateOneTimePad() {
    varOneTimePadText = inputOneTimePad.value;
    varOneTimePadKey = generateKey(varOneTimePadText.length, 26);
    let varOneTimePadCiphertext = shiftLetters(varOneTimePadText, varOneTimePadKey);
    canvasOneTimePad.clearRect(0, 0, WIDTH, 500);
    drawLetterChain(canvasOneTimePad, varOneTimePadText);
    drawLetterChain(canvasOneTimePad, varOneTimePadCiphertext, 275);
    drawShiftArrows(canvasOneTimePad, varOneTimePadKey, Math.min(varOneTimePadText.length, LETTERBOXLENGTH));
    spanOneTimePad.innerHTML = varOneTimePadKey;
}
updateOneTimePad();


const varOTPReuseSize = 32
...
function updateVernam() {
    ...
    let varVernamPlaintext = XORStringArray(varVernamCiphertext, varVernamKey);
    canvasVernam.clearRect(0, 0, WIDTH, 650);
    canvasVernamXOR.clearRect(0, 0, WIDTH, 650);
    drawLetterChain(canvasVernam, varVernamText);
    drawShiftArrows(canvasVernam, varVernamKey, Math.min(varVernamText.length, LETTERBOXLENGTH));
    drawLetterChain(canvasVernam, varVernamCiphertext, 275);
    drawShiftArrows(canvasVernam, varVernamKey, Math.min(varVernamText.length, LETTERBOXLENGTH), "+", 395);
    drawLetterChain(canvasVernam, varVernamPlaintext, 535);
    ...
    drawLetterChain(canvasVernamXOR, varVernamPlaintext, 535);
    spanVernam.innerHTML = varVernamKey;
    spanVernamXOR.innerHTML = varVernamKey;
}
updateVernam();


const arrayVernamReuseSize = 32;
...
const arrayVernamReuseBlocks = [arrayVernamReuseData1, arrayVernamReuseData2, arrayVernamReuseKey, arrayVernamReuseKey, arrayVernamReuseXOR1, arrayVernamReuseXOR1, arrayVernamReuseLeak];
const arrayVernamReuseBlockPos = [[15, 15], [375, 15], [15, 375], [375, 375], [15, 785], [375, 785], [885, 785]];
for (let i = 0; i < arrayVernamReuseBlocks.length; i++)
    drawChaChaBlocks(canvasVernamReuse, arrayVernamReuseBlocks[i], arrayVernamReuseBlockPos[i][0], arrayVernamReuseBlockPos[i][1], 10, "#000", "#000", "#000", 2);
...
for (let i = 0; i < 2; i++) {
    canvasKeyExchange.fillText("f", 325, 213 + i * 600);
...
function drawChaChaBlockArrow(canvas, block, x = 15, y = 15, size = 10, arrowStart = 415, arrowEnd = 575, color = "#000", inline = "#000", outline = "#000") {
    drawChaChaBlock(canvas, block, x, y, size, color, inline, outline);
    drawArrow(canvas, false, 1, y + 8 * size, arrowStart, arrowEnd);
}

function drawChaChaCountNonce(canvas, key, encryptionStreams, counter = 0, nonce = 0, x = 15, y = 15, sizeStream = 6, streamsOffset = 240, streamGroupOffset = 70, blockOffset = 640, sizeBlock = 10, lineWidth = 4) {
    const initBlock = createChaChaInitBlock(key, counter, nonce);
    drawChaChaBlockArrow(canvas, initBlock, x, y, sizeBlock, x + 32 * sizeBlock + 80, x + blockOffset - 80);
...
function drawChaChaEncryptionFromKey(canvas, initBlock, dataStream, chachaStream, xorStream, x = 15, y = 15, keyOffset = 390, size = 8) {
    drawChaChaBlockArrow(canvas, initBlock, x, y + size * 19, size, x + size * 32 + 20, x + keyOffset - 20);
...
function shiftLetters(plaintext, key, sign = 1) {
    ...
    for (let i = 0; i < plaintext.length; i++) {
        ...
        if (currentChar.charCodeAt(0) >= 65 && currentChar.charCodeAt(0) < 91)
            encryptedChar = String.fromCharCode(((currentChar.charCodeAt(0) - 65 + sign * currentShift) % 26) + 65);
