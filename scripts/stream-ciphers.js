...
const canvasKeyExchangeChaCha = initializeCanvas("canvasKeyExchangeChaCha", 1080);
drawArrow(canvasKeyExchange, true, 1, 143, 325, 737);
...
canvasKeyExchangeChaCha.drawImage(canvasKeyExchange.canvas, 0, 0);
initializeCanvasText(canvasKeyExchange, "left");
initializeCanvasText(canvasKeyExchangeChaCha, "left");
for (let i = 0; i < 2; i++) {
    canvasKeyExchange.fillText(PARTYLABELS[i], 15, 135 + i * 600);
    canvasKeyExchangeChaCha.fillText("Seed", 15, 135 + i * 600);
    canvasKeyExchangeChaCha.fillText("f", 329, 209 + i * 600);
}
...
const arrayChaChaPartitionSequenceInit = [...EG_CHACHAINITBLOCK];
const canvasChaChaPartitionSequence = initializeCanvas("canvasChaChaPartitionSequence", 414);
const buttonChaChaPartitionSequenceIncrement = document.getElementById("buttonChaChaPartitionSequenceIncrement");
const buttonChaChaPartitionSequenceDecrement = document.getElementById("buttonChaChaPartitionSequenceDecrement");
buttonChaChaPartitionSequenceIncrement.addEventListener("click", () => {
    arrayChaChaPartitionSequenceInit[13] += 1;
    updateChaChaPartitionSequence(); });
buttonChaChaPartitionSequenceDecrement.addEventListener("click", () => {
    if (arrayChaChaPartitionSequenceInit[13] <= 0) return;
    arrayChaChaPartitionSequenceInit[13] -= 1;
    updateChaChaPartitionSequence(); });
function updateChaChaPartitionSequence() {
    canvasChaChaPartitionSequence.clearRect(0, 0, WIDTH, 414);
    drawChaChaBlock(canvasChaChaPartitionSequence, arrayChaChaPartitionSequenceInit);
...
const canvasChaChaNonce = initializeCanvas("canvasChaChaNonce", 1136);
...
for (let i = 0; i < 2; i++) {
    ...
    structChaChaNonce[i] = createEncryptionStreams(arrayInitBlock, 7, EG_DATA_STREAMS[i]);
}
const arrayChaChaNonceXOR = createXORStream(structChaChaNonce[0].xorStream, structChaChaNonce[1].xorStream);
...
function updateChaChaNonce() {
    canvasChaChaNonce.clearRect(0, 0, WIDTH, 1140);
    drawChaChaCountNonce(canvasChaChaNonce, EG_CHACHAKEY, structChaChaNonce, varChaChaNonceCounter, varChaChaNonce, 15, 15, 6, 220, 66);
    drawChaChaBlocks(canvasChaChaNonce, arrayChaChaNonceXOR, 15, 1027, 6);
...
const arrayStructuredKeyExchangeLabels = ["Key", "Nonce"];
...
const AUTH_POLYNOMIAL_ORIG = getPolynomialExpression(AUTH_CIPHERSTREAM_ORIG);
const AUTH_POLYNOMIAL_MODF = getPolynomialExpression(AUTH_CIPHERSTREAM_MODF);
const AUTH_POLYNOMIAL_EQNS = [[AUTH_POLYNOMIAL_ORIG + " ≡ ", 740, 569], ["=> x = ", 740, 609], [AUTH_POLYNOMIAL_MODF + " ≡ ", 740, 837]];
...
const SEEDHASHARROWS = [[[405, 157], [661, 157], [533, 157], [533, 3], [1229, 3], [1229, 269]], [[405, 757], [661, 757], [533, 757], [533, 603], [1199, 603]]];
...
const arrayMalhashHashes = [[AUTH_CHECKSUM_ORIG, 1119, 383], [AUTH_CHECKSUM_MODF, 1119, 1023]];
const arrayMalhashHashesRaw = [[toHexString(AUTH_CHECKSUM_CIPHER_ORIG) + " (-)", 740, 559], [toHexString(AUTH_CHECKSUM_CIPHER_MODF) + " (+)", 740, 847]];
...
for (let i = 0; i < arrayMalhashHashesRaw.length; i++)
    drawHash(canvasMalhash, arrayMalhashHashesRaw[i][0], arrayMalhashHashesRaw[i][1], arrayMalhashHashesRaw[i][2], [], false, true);
...
const varPolyhashKey = Math.floor(Math.random() * 0xFFFFFFFF);
const canvasPolyhash = initializeCanvas("canvasPolyhash", 1410);
const varPolyhashOrig = calculateHash(AUTH_CIPHERSTREAM_ORIG, varPolyhashKey);
const varPolyhashModf = calculateHash(AUTH_CIPHERSTREAM_MODF, varPolyhashKey);
const arrayPolyhashTexts = [[varPolyhashOrig, 740, 569], [varPolyhashKey, 740, 609], [varPolyhashModf, 740, 837], [varPolyhashModf, 1119, 973]];
drawChaChaEncryption(canvasPolyhash, AUTH_DATASTREAM_ORIG, AUTH_KEYSTREAM, AUTH_CIPHERSTREAM_ORIG, 15, 15, 8);
drawChaChaEncryption(canvasPolyhash, AUTH_CIPHERSTREAM_MODF, AUTH_KEYSTREAM, AUTH_DATASTREAM_MODF, 15, 959, 8);
drawHash(canvasPolyhash, varPolyhashOrig, 1119, 383, [varPolyhashKey]);
drawHash(canvasPolyhash, varPolyhashModf, 1119, 1023);
for (let i = 0; i < arrayPolyhashTexts.length; i++)
    canvasPolyhash.fillText(toHexString(arrayPolyhashTexts[i][0]), arrayPolyhashTexts[i][1], arrayPolyhashTexts[i][2]);
initializeCanvasText(canvasPolyhash, "right");
for (let i = 0; i < AUTH_POLYNOMIAL_EQNS.length; i++)
    canvasPolyhash.fillText(AUTH_POLYNOMIAL_EQNS[i][0], AUTH_POLYNOMIAL_EQNS[i][1], AUTH_POLYNOMIAL_EQNS[i][2]);
drawOrthoArrow(canvasPolyhash, [[1189, 423], [1189, 569], [913, 569]]);
drawOrthoArrow(canvasPolyhash, [[913, 837], [1189, 837], [1189, 933]]);
drawOrthoArrow(canvasPolyhash, [[527, 469], [527, 539]]);
drawOrthoArrow(canvasPolyhash, [[527, 939], [527, 867]]);
drawOrthoArrow(canvasPolyhash, [[721, 639], [721, 715], [455, 715], [455, 807]]);
drawLock(canvasPolyhash, 1101, 702);
canvasPolyhash.moveTo(1073, 383); canvasPolyhash.lineTo(1073, 1023);
...
const varFailhashPad = Math.floor(Math.random() * 0xFFFFFFFF);
const canvasFailhash = initializeCanvas("canvasFailhash", 1410);
const varFailhashOrig = calculateHash(AUTH_CIPHERSTREAM_ORIG, varFailhashKey, varFailhashPad);
const varFailhashModf = calculateHash(AUTH_CIPHERSTREAM_MODF, varFailhashKey, varFailhashPad);
const arrayFailhashTexts = [["??????????", 740, 569], ["??????????", 740, 609], ["??????????", 740, 837], ["??????????", 1119, 973]];
...
drawHash(canvasFailhash, varFailhashOrig, 1119, 383, [varFailhashKey, varFailhashPad]);
...
initializeCanvasText(canvasFailhash, "right");
for (let i = 0; i < AUTH_POLYNOMIAL_EQNS.length; i++)
    canvasFailhash.fillText(AUTH_POLYNOMIAL_EQNS[i][0], AUTH_POLYNOMIAL_EQNS[i][1], AUTH_POLYNOMIAL_EQNS[i][2]);
drawOrthoArrow(canvasFailhash, [[1189, 423], [1189, 569], [913, 569]]);
drawOrthoArrow(canvasFailhash, [[913, 837], [1189, 837], [1189, 933]]);
drawOrthoArrow(canvasFailhash, [[527, 469], [527, 539]]);
drawOrthoArrow(canvasFailhash, [[527, 939], [527, 867]]);
drawOrthoArrow(canvasFailhash, [[721, 639], [721, 715], [455, 715], [455, 807]]);
...
spanMachashHashPadded.innerHTML = "...";
...
inputMachashKey.addEventListener("input", () => { resetHashGeneral(updateMachash, arrayMachashPointers, inputMachashKey, inputMachashKeyPad, spanMachashHashPadded) });
inputMachashKeyPad.addEventListener("input", () => { resetHashGeneral(updateMachash, arrayMachashPointers, inputMachashKey, inputMachashKeyPad, spanMachashHashPadded) });
function updateMachash() {
    ...
    if (arrayMachashPointers[0] == AUTH_CIPHERSTREAM_ORIG_SHORT.length - 1)
...
spanPoly1305HashPadded.innerHTML = "...";
...
function updatePoly1305() {
    ...
    if (varPoly1305Pointer * 4 == AUTH_CIPHERSTREAM_ORIG_SHORT.length - 4)
        spanPoly1305HashPadded.innerHTML = toHexStringBigInt(BigInt.asUintN(128, varPoly1305CurrentHash + varPoly1305KeyS)) + " [h + k]";
    varPoly1305Pointer++;
}
updatePoly1305();


let varPrimeFieldModulus = 24;
let varPrimeFieldMultiplier = 6;
let arrayPrimeFieldLabels = [];
let arrayPrimeFieldDist = [];
let arrayPrimeFieldUDist = [];
...
const linkPrimeFieldComposite = document.getElementById("linkPrimeFieldComposite");
const linkPrimeFieldPrime = document.getElementById("linkPrimeFieldPrime");
...
linkPrimeFieldComposite.addEventListener("click", () => {
    sliderPrimeFieldModulus.value = 30;
    sliderPrimeFieldMultiplier.value = 6;
    updatePrimeField();
});
linkPrimeFieldPrime.addEventListener("click", () => {
    sliderPrimeFieldModulus.value = 31;
    sliderPrimeFieldMultiplier.value = 6;
    updatePrimeField();
});
...
const canvasSecretsExchange = initializeCanvas("canvasSecretsExchange", 1080);
...
initializeCanvasText(canvasSecretsExchange, "left");
drawChaChaEncryptionFromKey(canvasSecretsExchange, EG_CHACHAINITBLOCK, AUTH_DATASTREAM_ORIG_SHORT, arrayChaChaPolyKeystream1, arrayChaChaPolyCipherstream, 15, 15, 646);
drawChaChaEncryptionFromKey(canvasSecretsExchange, EG_CHACHAINITBLOCK, arrayChaChaPolyCipherstream, arrayChaChaPolyKeystream1, AUTH_DATASTREAM_ORIG_SHORT, 15, 615, 646);
drawLock(canvasSecretsExchange, 125, 533);
canvasSecretsExchange.fillText("Seed", 145, 533);
drawLock(canvasSecretsExchange, 1231, 503, 0);
drawOrthoArrow(canvasSecretsExchange, [[1263, 405], [1263, 645]]);
for (let i = 0; i < 2; i++) {
...
const arrayChaChaPolyIntegratedLabels = ["ChaCha Key", "Nonce, Hash"];
...
drawLock(canvasChaChaPolyIntegrated, 1231, 509, 0);
drawOrthoArrow(canvasChaChaPolyIntegrated, [[1263, 405], [1263, 645]]);
for (let i = 0; i < 2; i++) {
...
const POLYMODULUS = 31;  // ensure p % 4 = 3
let arrayPolynomial1Values = [];
const arrayPolynomial1Sliders = ["sliderPolynomial1Coefficient1", "sliderPolynomial1Range"];
const arrayPolynomial1Spans = ["spanPolynomial1Coefficient1", "spanPolynomial1Range"];
...
function updatePolynomial1() {
    ...
    spanPolynomial1Equation.innerHTML = `${arrayPolynomial1Values[0]}⋅x ≡ ${arrayPolynomial1Values[1]} (mod) ${POLYMODULUS}`;
    spanPolynomial1Root.innerHTML = `=> x = [${getRootLinear(arrayPolynomial1Values[0], -arrayPolynomial1Values[1], POLYMODULUS)}]`;
}
updatePolynomial1();


let arrayPolynomial2Values = [];
const arrayPolynomial2Sliders = ["sliderPolynomial2Coefficient1", "sliderPolynomial2Coefficient2", "sliderPolynomial2Range"];
const arrayPolynomial2Spans = ["spanPolynomial2Coefficient1", "spanPolynomial2Coefficient2", "spanPolynomial2Range"];
...
function updatePolynomial2() {
    ...
    spanPolynomial2Equation.innerHTML = `${arrayPolynomial2Values[0]}⋅x^2 + ${arrayPolynomial2Values[1]}⋅x ≡ ${arrayPolynomial2Values[2]} (mod) ${POLYMODULUS}`;
    spanPolynomial2Root.innerHTML = `=> x = [${getRootQuadratic(arrayPolynomial2Values[0], arrayPolynomial2Values[1], -arrayPolynomial2Values[2], POLYMODULUS)}]`;
}
updatePolynomial2();
...
function getRootQuadratic(a, b, c, prime) {
    let discriminant = ((((b * b) - (4 * a * c)) % prime) + prime) % prime;
    let inverseA = getFieldInverse(2 * a, prime);
    if (discriminant == 0)
        return [((prime - b) * inverseA) % prime]
    let sqroots = getSquareRootField(discriminant, prime);
    if ((sqroots[0] * sqroots[0]) % prime != discriminant)
        return [];
    return [((((sqroots[0] - b) * inverseA) % prime) + prime) % prime, ((((sqroots[1] - b) * inverseA) % prime) + prime) % prime];
}

function getRootLinear(a, b, prime) {
    return ((prime - b) * getFieldInverse(a, prime)) % prime;
}

function getFieldInverse(number, prime) {
    let inverse = number;
    for (let i = 1; i < prime - 2; i++)
        inverse = (inverse * number) % prime;
    return inverse;
}

function getSquareRootField(number, prime) { // use p % 4 = 3
    let root = number;
    for (let i = 1; i < (prime + 1)/4; i++)
        root = (root * number) % prime;
    return [root, (((prime - root) % prime) + prime) % prime];
}
...
function getPolynomialExpression(bytes, length = 2, indeterminate = "x") {
    let equation = "...";
    let end = bytes.length - 1;
    for (let i = length - 1; i >= 0; i--)
        equation += " + " + toHexString(bytes[end - i]) + "*" + indeterminate + "^" + (i + 1);
    return equation;
}
...
function initializePolynomialIllustration(sliderIDs, spanIDs, updateFunction, modulus) {
    ...
    for (let i = 0; i < sliderIDs.length; i++) {
        const slider = initializeSliders(sliderIDs[i], (i === 0 || i === sliderIDs.length - 1) ? 1 : 0, modulus - 1, 1, Math.floor(Math.random() * modulus));
...
function resetHashGeneral(updateFunction, pointers, inputKey = false, inputPad = false, spanPad = false) {
...
function drawMultiplyModuloMap(canvas, modulus, multiplier, height = 120, xOffset = 15, yOffset = 185, color = "#aaa") {
    ...
    canvas.beginPath();
    for (let i = 0; i < modulus; i++) {
        const output = (i * multiplier) % modulus;
        canvas.moveTo(offset + i * 1248 / modulus, yOffset);
        canvas.lineTo(offset + output * 1248 / modulus, yOffset + height);
    }
    canvas.stroke();
}
