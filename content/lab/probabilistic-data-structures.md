...
It might seem that it using multiple hash functions would reduce errors as well, similar to bloom filters. However in counting bloom filters, increasing the number of hash functions does not decrease errors — in fact it actually leads to *slightly* higher errors.
...
There is a glaring flaw in this approach however. The gaps between the approximations double every time an extra consecutive zero is observed — it can only provide estimates that are powers of two, and nothing in between. Second, a hash output may have a lot of consecutive zeros at the beginning simply owing to chance, resulting in overestimation of the cardinality.

To mitigate these problems, multiple counters can be used that store the longest run of zeroes for different subsets of the multiset — instead of storing one counter that stores the longest run of zeroes for the entire multiset. The values can then be averaged to find a more precise cardinality estimate.
...
Acutal cardinality: <span id="hllOutputBucketCardinality">0</span><br>
...
However, even with the corrective measures, there is a [predictable bias](https://www.moderndescartes.com/essays/hyperloglog/#loglog) towards larger estimates. Scaled the average by a correction factor counteracts this bias. This can bring down the [error](https://en.wikipedia.org/wiki/Standard_error) to 1.04/√m, where m is the number of buckets.
...
Acutal cardinality: <span id="hllOutputMainCardinality">0</span><br>
...
The correction factor ranges between 0.672 and 0.709, depending on the number of buckets. It is approximately equal to `0.723/(1+1.079/m)` where `m` is the number of buckets.
{% end %}

This is HyperLogLog. A very simple and elegant data structure that is also incredibly efficient. Adding elements involves hashing them and storing their longest run of leading zeroes in buckets. Estimating the cardinality involves finding the harmonic mean of the estimates, scaled by the number of buckets and a correction factor.
