...
As mentioned earlier, sometimes lights with different spectral power distributions can produce similar responses in the cones as spectral colours. But the same is also true for non-spectral colours. More broadly, the same cone responses can be produced by light having different spectral power distributions. This means different types of lights can appear to have the same colour. This phenomena of light with different spectral power distributions having the colours is called [metamerism](https://en.wikipedia.org/wiki/Metamerism_(colour)).
...
This phenomenon was explored more in colour matching experiments by [William David Wright](https://doi.org/10.1088/1475-4878/30/4/301) and [John Guild](https://doi.org/10.1098/rsta.1932.0005). Light with three wavelengths (435nm, 546nm, and 700nm) of differing intensities were mapped to spectral colours by varying the intensities of its constituent monochromatic lights such that the resultant light was perceived to be the same as the spectral colour. The findings were then summarized, and the observations resulted in (the now standardized) colour-matching curves.

The colour matching curves define the intensity of each of the [primaries](https://en.wikipedia.org/wiki/Primary_color#Real_primaries) (colours used to recreate other colours) required to replicate the spectral colour at that wavelength.
...
The CIE RGB colour space uses as lights of 700nm (R), 546nm (G), and 435nm (B) as its primaries. The LMS colour space, in contrast, used the response of the cones (L,M,S) as its bases.
{% end %}
