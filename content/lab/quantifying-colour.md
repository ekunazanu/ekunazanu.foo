...
The aggregate of the majority can be slll.
{% end %}

### Non-Spectral Colours

Until now, only [spectral colours](https://en.wikipedia.org/wiki/Spectral_color) (colours corresponding to monochromatic light) have been discussed. But most of the light around us is not monochromatic, it is a combination of multiple wavelengths of light. This slightly complicates the measurement of the cone responses however. Earlier, the sensitivity of the cones at the given wavelength was simply used, but this is not possible now since there is no single specific wavelength, but there are multiple — and the intensity at different wavelengths need not be the same (The monochromatic light had the maximum intensity in all the cases above).
...
Mathematically, and more formally framed, if `J(λ)` describes the spectral power distribution, and `l(λ)`, `m(λ)`, and `s(λ)` are the sensitivity curves of the L-cones, M-cones, and S-cones, then the L, M, and S values (responses) are:

`L = ∫ J(λ)·l(λ)·dλ`\
`M = ∫ J(λ)·m(λ)·dλ`\
`S = ∫ J(λ)·s(λ)·dλ`

These responses are then normalized such that their maxima is equal to unity.

So the colour of any light or any object reflecting light can be precisely described by its (L,M,S) values — which can be derived by from its spectral power distribution. The set of all possible (L,M,S) values describes every perceivable colour, and all these possible values together form a three dimensional space, aptly called a [colour space](https://en.wikipedia.org/wiki/Color_space). More specifically, this is the [LMS colour space](https://en.wikipedia.org/wiki/LMS_color_space), where colours are defined as a set of (L,M,S) values. The LMS colour space here is visualized below, where each of the responses of the of the cones is represented using a spatial dimension.
...
So goal achieved. There is now a way to quantify colours precisely, using numbers. Except this is not at all the standard used for describing colours. The LMS colour space is *one* way to represent colours, but is not the *standard* way to represent them.

The LMS colour space depends on the sensitivity curves of the cones. However, the process/need of quantifying colours arose way before the sensitivity of the cones was measurable..
...
The responses of the cones cannot be calculated from the spectral power distribution if there is **no** relationship defined between the two.
{% end %}

. Generally, the standard reference is the [CIE 1931 colour space](https://en.wikipedia.org/wiki/CIE_1931_color_space). The reason the LMS colour space is not the standard reference colour space is because there already was a standard reference for colours — the CIE colour space. In fact, it was created even before the response curves of the cones were accurately mapped. The pre-existing standard reference was created using colour matching experiments in the earlier decades. These experiments formed the basis for a standard reference framework to compare and quantify different colours.

So a completely different way of quantifying colours was used that does not use LMS responses as its basis.
