...
From the twelve notes of an octave, a subset can be selected to get a [scale](https://en.wikipedia.org/wiki/Scale_(music)). Scales are defined their intervals and notes — to set a specific 'mood' for the sounds. Usually music in a [major scale](https://en.wikipedia.org/wiki/Major_scale) sounds upbeat, while music in [minor scales](https://en.wikipedia.org/wiki/Minor_scale) sound melancholic. The [pentatonic scale](https://en.wikipedia.org/wiki/Pentatonic_scale) has an interval structure with five notes. A [mode](https://en.wikipedia.org/wiki/Mode_(music)) is a kind of generalized scale; it has an interval structure but does not any specific [key](https://en.wikipedia.org/wiki/Key_(music)) (root note).
...
The amplitude, just like the pitch, can also be altered to affect the character of a sound. <mark>The loudness of a section can ascend (crescedo) to create drama and tension, or it can gradually descend (decrescendo) after resolving the tension. Sounds can also be soft and quiet (piano) to emphasize the pieces that are strong and loud (forte) following them. The loudness is called [dynamics](https://en.wikipedia.org/wiki/Dynamics_(music)).</mark>
...
The amalgamation of all the previously mentioned parameters can lead t .
